...
public class SeedService(AppDbContext db, ILogger<SeedService> logger) : ISeedService
{
    ...
    public async Task EnsureSchemaAsync(CancellationToken ct = default)
    {
        ...
        await EnsureLateAddedTablesAsync(ct);
    ...
    /// <summary>
    /// Veritabanı kurulduktan SONRA modele eklenen tablolar.
    ///
    /// EnsureCreated yalnızca tamamen boş bir veritabanında tablo yaratır. Şema bir
    /// kez kurulduktan sonra modele yeni bir tablo eklenirse EnsureCreated onu fark
    /// etmez ve tablo hiç oluşmaz; o tabloya giden ilk sorgu da 42P01 ("relation does
    /// not exist") ile patlar. Aşağıdaki DDL bu yüzden burada ve idempotent: var olan
    /// veritabanını bozmadan eksik tabloyu tamamlar.
    /// </summary>
    private async Task EnsureLateAddedTablesAsync(CancellationToken ct)
    {
        await db.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS perfume_user_photos (
                ...
                CONSTRAINT pk_perfume_user_photos PRIMARY KEY (id),
                CONSTRAINT fk_perfume_user_photos_perfumes_perfume_id
                    FOREIGN KEY (perfume_id) REFERENCES perfumes (id) ON DELETE CASCADE,
                CONSTRAINT fk_perfume_user_photos_users_user_id
                    FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE
            );

            CREATE INDEX IF NOT EXISTS ix_perfume_user_photos_perfume_id
                ON perfume_user_photos (perfume_id);
            CREATE INDEX IF NOT EXISTS ix_perfume_user_photos_user_id
                ON perfume_user_photos (user_id);
            """, ct);
    }
